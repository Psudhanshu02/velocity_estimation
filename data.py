...
def download_data(directory) -> None:
    ...
    os.system(f"/home/fatninja/Desktop/projects/velocityEstimation/velocity_estimation/data/train.mp4")
    os.system(f"/home/fatninja/Desktop/projects/velocityEstimation/velocity_estimation/data/train.txt")
...
class MultiFrameDepthVideo(MultiFrameVideo):
    ...
    def get_multiple_images(self, index):
        ...
        for i in range(index, index+self.depth):
            x1s.append(self._img(index).T)#transpose image
    ...
    def __getitem__(self, index):
        ...
        # return the mean of the two speeds from
        # each frame
        _y = self.Y[index:index+(self.depth)]
        _y2 = self.Y[index+ self.frame_delta:index+(self.frame_delta + self.depth)]
